...
impl IntoResponse for ApiError {
    fn into_response(self) -> Response {
        let info = match &self {
            ...
            ApiError::Share(ShareError::TaskNotFound(_))
            | ApiError::Share(ShareError::ProjectNotFound(_)) => {
                ErrorInfo::not_found("ShareError", "Resource not found.")
            }
            ApiError::Share(ShareError::AlreadyShared(_)) => {
                ErrorInfo::conflict("ShareError", "Task is already shared.")
            }
            ApiError::Share(ShareError::MissingAuth) => ErrorInfo::with_status(
                StatusCode::UNAUTHORIZED,
                "ShareError",
                "Authentication required for sharing.",
            ),
            ApiError::Share(ShareError::MissingConfig(_)) => {
                ErrorInfo::bad_request("ShareError", "Missing configuration for sharing.")
            }
            ApiError::Share(_) => ErrorInfo::internal("ShareError"),
        };
